...
def seed_database(db):
    ...
    logger.info(f"Seeded {len(satellites)} satellites and {len(debris_objects)} debris objects with fallback TLEs")
...
def ensure_fallback_tles(db):
    ...
    for sat in all_sats:
        has_tle = db.query(TLERecord).filter(TLERecord.norad_id == sat.norad_id).first()
    ...
    for deb in all_debs:
        has_tle = db.query(TLERecord).filter(TLERecord.norad_id == deb.norad_id).first()
